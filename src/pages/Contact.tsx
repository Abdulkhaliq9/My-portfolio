...
const Contact = () => {
  ...
  const contactLinks = [
    { href: `mailto:${contactContent.email}`, icon: Mail, label: "Email", text: contactContent.email },
    { href: contactContent.linkedin, icon: Linkedin, label: "LinkedIn", text: "Connect with me", external: true },
    { href: contactContent.github, icon: Github, label: "GitHub", text: "View my code", external: true },
    { href: contactContent.resumeUrl, icon: Download, label: "Resume", text: "Download CV" },
  ];
      ...
      <section className="section-container relative z-10">
        ...
        >
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-4 sm:mb-6">
            ...
            <span className="gradient-text">{contactContent.heading.split(" ").slice(2).join(" ")}</span>
          </h1>
          <p className="text-base sm:text-lg text-muted-foreground max-w-2xl">{contactContent.subheading}</p>
        </motion.div>
      </section>
            ...
            <ScrollReveal delay={0.15} direction="right">
              <div className="space-y-3 sm:space-y-4">
                {contactLinks.map((link, i) => (
                  <ScrollReveal key={link.label} delay={0.1 * i} direction="right">
                    <a
                      href={link.href}
                ...
                <ScrollReveal delay={0.5}>
                  <div className="glass-card p-6 sm:p-8 text-center hover-glow mt-4 sm:mt-6">
                    <p className="text-sm text-muted-foreground mb-2">Prefer a quick call?</p>
                    <p className="text-foreground font-medium text-sm sm:text-base">Schedule a meeting to discuss your project</p>
                  </div>
                </ScrollReveal>
              </div>
            </ScrollReveal>
